...
function SidebarNav({
  ...
  // Filter nav items by role using the central MODULE_VISIBILITY table.
  // This is the SINGLE source of truth — students no longer see admin
  // modules (Élèves, Enseignants, Personnel, Paramètres, etc.).
  // Previously, ALL modules were shown to ALL roles, which made the
  // student dashboard look identical to the admin dashboard.
  const visibleItems = NAV_ITEMS.filter((item) =>
    isModuleVisible(userRole, item.key)
  )
  return (
...
export default function AppShell() {
  ...
  // Guard: if the activeModule is not visible for the current user's role
  // (e.g. a student whose persisted activeModule is 'students' from a
  // previous admin session), reset to 'dashboard'. This prevents a student
  // from landing on an admin-only module after login.
  useEffect(() => {
    if (!currentUser?.role) return
    if (!isModuleVisible(currentUser.role, activeModule)) {
  ...
  // Logout handler — clears ALL client caches (Service Worker Cache API,
  // IndexedDB offline cache, in-flight API dedupe map) BEFORE resetting the
  // store, then HARD RELOADS the page to guarantee a 100% clean state.
  //
  // The hard reload is critical: even after clearing the caches, a stale
  // Service Worker (e.g. on production where the new SW hasn't activated
  // yet) could still serve cached API responses from the current user to
  // the next user who logs in. A hard reload forces the browser to
  // re-fetch everything from the network, activates any pending new SW,
  // and re-bootstraps the React app from the persisted Zustand state
  // (which now has currentUser=null, isAuthenticated=false → renders Login).
  const handleLogout = useCallback(async () => {
    await clearAllCaches()
    logout()
    // Hard reload to destroy all in-memory React state and force the
    // browser/SW to start fresh.
