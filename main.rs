...
//
// IMPORTANT: All server stdout/stderr is written to a log file so users can
// diagnose startup failures (Node missing, Prisma error, port conflict, etc.).
// Log file location:
...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            // Detect which runtime is available: prefer node, fall back to bun
            let (cmd_name, runtime_label) = if Command::new("node")
                .arg("--version")
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status()
                ...
                .is_ok()
            {
                ...
                log_line(&log_path, "ERROR: neither 'node' nor 'bun' found in PATH!");
                log_line(&log_path, "Please install Node.js LTS from https://nodejs.org/");
                return Ok(());
            };
            log_line(&log_path, &format!("Using runtime: {}", runtime_label));
            ...
            let child = match child {
                Ok(c) => {
                    log_line(&log_path, &format!("Server process spawned (PID: {})", c.id()));
                    c
                }
                Err(e) => {
