...
function ensureEnv() {
  ...
  // 4. Inject .env vars into process.env.
  //    For DATABASE_URL, ALWAYS prefer the .env value over any pre-existing
  //    shell env var. This fixes sandboxes where the shell exports a stale
  //    SQLite URL (e.g. `file:/.../custom.db`) that doesn't match the
  //    `postgresql` provider declared in schema.prisma — which would
  //    otherwise cause a Prisma validation error ("the URL must start with
  //    the protocol postgresql://") on every DB query and break login.
