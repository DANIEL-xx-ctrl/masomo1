...
export default function Login() {
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (email === 'superadmin@edugest.com' || email === 'superadmin@masomo.com') {
          try {
            ...
            if (retryRes.ok && retryData.user) {
              // Clear ALL client caches so the new user never sees the
              // previous user's cached API data.
              await clearAllCaches()
              login(retryData.user)
              // Hard reload for a 100% clean state (see main login path).
      ...
      if (data.user) {
        // Clear ALL client caches (SW Cache API + IndexedDB + in-flight)
        // so the new user never sees the previous user's cached API data.
        await clearAllCaches()
        login(data.user)
        ...
        // Celebrate! Fire confetti BEFORE the hard reload — the canvas
        // is appended to document.body so it shows briefly.
        setTimeout(() => fireConfetti({ duration: 2400, count: 140, originY: 0.4 }), 80)
        // HARD RELOAD to guarantee a 100% clean state. This is the critical
        // fix for the "old login remains after logout" bug. Even though we
        // cleared the caches above, a stale Service Worker (e.g. on
        // production where the new SW hasn't activated yet) could still
        // serve cached API responses from the previous user. A hard reload
        // forces the browser to re-fetch everything from the network,
        // activates any pending new SW, and re-bootstraps the React app
        // from the persisted Zustand state (which now has the new user).
        // The localStorage is written synchronously by Zustand persist,
        // so the reloaded page will immediately see isAuthenticated=true
        // and render the AppShell with the correct user.
        setTimeout(() => { window.location.href = '/' }, 350)
      } else {
